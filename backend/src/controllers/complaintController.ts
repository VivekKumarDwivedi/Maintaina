import { WhereOptions, Op, QueryTypes } from "sequelize";
...
const getMyComplaints = async (req: AuthRequest, res: Response) => {
  try {
    const complaints = await Complaint.findAll({
      ...
      order: [
        ["createdAt", "DESC"],
        [{ model: ComplaintHistory, as: "history" }, "createdAt", "ASC"],
      ],
    });
    res.json(complaints);
  } catch (err: unknown) {
      ...
      include: [
        {
          model: User,
          as: "resident",
          attributes: ["id", "name", "email", "flatNumber"],
        },
        {
          model: ComplaintHistory,
          as: "history",
...
const updateStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (complaint.status === "Closed")
      return res
        .status(400)
        .json({ error: "Closed complaints cannot be updated" });
...
const flagOverdue = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({
      flagged: complaints.length,
      message: `${complaints.length} complaint(s) flagged as overdue`,
    });
  } catch (err: unknown) {
    res
      .status(500)
